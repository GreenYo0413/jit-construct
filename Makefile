...
jit0-x64-O: tests/jit0-x64-O.c
	$(CC) $(CFLAGS) -o $@ $^

jit-x64-O: dynasm-driver.c jit-x64-O.h
	$(CC) $(CFLAGS) -o $@ -DJIT=\"jit-x64-O.h\" \
		dynasm-driver.c
jit-x64-O.h: jit-x64-O.dasc
	        $(LUA) dynasm/dynasm.lua -o $@ jit-x64-O.dasc
run-jit-x64-O: jit-x64-O
	./jit-x64-O progs/hello.b && objdump -D -b binary \
		-mi386 -Mx86-64 /tmp/jitcode
